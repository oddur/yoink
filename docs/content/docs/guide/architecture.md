...
- **`age`** (default, batteries-included) — yoink decrypts a sealed `secrets.age` file at deploy time using one X25519 identity, resolved in this order: `YOINK_AGE_KEY` (raw key, for CI) → `YOINK_AGE_KEY_FILE` (explicit path override) → `~/.config/yoink/keys/<recipient>.key` (the dir scanned for a key whose public matches one of `secrets.recipients:` — where `yoink secrets key generate` saves by default; multiple projects coexist here without env-var dance) → `~/.config/yoink/age.key` (legacy single-key fallback). On a laptop, `yoink secrets key generate` writes to the keys dir and yoink finds it automatically. For CI, `yoink secrets key generate --print | gh secret set YOINK_AGE_KEY` pipes the secret straight into your store; surface it as `YOINK_AGE_KEY` at deploy time.
