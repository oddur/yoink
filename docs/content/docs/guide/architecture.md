...
- If the container is already running on a host, `up` errors with "another deploy in progress." The competing operator either waits or (if the sentinel is stale) re-runs after the sentinel self-exits.
- If the container exists but is stopped (a crashed previous deploy), yoink force-removes it and starts fresh.
...
Both providers produce the same shape: a key→value bundle. Every service that lists `secrets:` (or `env_from_secrets:`) gets its values picked out of that bundle and injected as env vars on the container. Those values feed into `yoink.spec_hash`, which is why a rotated secret triggers a redeploy.
