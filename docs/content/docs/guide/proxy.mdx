---
title: Reverse proxy
description: "The bundled Caddy proxy: one field to expose a service over HTTPS with automatic certs or sealed Cloudflare origin certs."
weight: 4
---
...
Point `api.example.com` DNS at `1.2.3.4`. Run `yoink up`. `https://api.example.com` serves with a valid cert.

The rest of this page is reference + advanced patterns.
...
When any service has `domain:` set, yoink synthesizes a `yoink-proxy` service running `caddy:2` and renders its config from your `yoink.yaml`. The config is pushed via Caddy's admin API at deterministic points in the rolling deploy; yoink owns the routing flip with no Docker-label fights with a separate reconciler.
...
On the next `yoink up`, each proxy host runs a one-shot xcaddy build (multi-stage `caddy:2-builder` → `caddy:2`) and tags the result locally as `yoink-caddy:<hash>`. The proxy service runs from that tag with no registry needed. The hash is content-addressed over the build inputs, so subsequent `yoink up` runs short-circuit (`image_present` skip) until plugins or version change.
...
Five service-level fields, four proxy-level fields. Anything beyond that is `caddy_extra_json:` / `caddy_extra_caddyfile:` (next section) or a custom proxy image.

## Cert volume: keep it

The `yoink_caddy_data` named volume holds your ACME state, issued certs, and OCSP staples. Don't delete it casually: Let's Encrypt rate-limits aggressively (5 duplicate-cert issuances per week) and a fresh volume means re-issuance.
...
3. Yoink pushes updated Caddy config (`/load`), adding the new replica to the upstream pool.
...
The cookbook of ready-to-paste snippets lives in its own page: [Caddy snippets cookbook](/docs/how-to/caddy-snippets). Each snippet shows both the Caddyfile and JSON forms for forward auth, basic auth, IP allowlist, custom headers, body-size limit, redirects, maintenance page, and plugins.
