...
description: Issue a 15-year Cloudflare Origin Certificate and lock your origin to Cloudflare's edge via Authenticated Origin Pulls.
weight: 13
---
...
- **15-year validity.** Rotate annually if you prefer; otherwise set a calendar reminder for the expiry.
...
The downside: only Cloudflare-fronted clients trust the cert. Origin certs are not for direct browser access.

## Setup


### Issue the cert in the Cloudflare dashboard [step]
...
### Add the cert + key to `secrets.age` [step]
...
**Many services, one wildcard cert** (`*.example.com` covers everything) via proxy-level config:
...
### Add the Origin Pull CA bundle to `secrets.age` [step]
...
### Enable Authenticated Origin Pulls in the Cloudflare dashboard [step]

SSL/TLS → Origin Server → **Authenticated Origin Pulls** → toggle on.

### Add `client_auth:` to `proxy.tls:` in `yoink.yaml` [step]
