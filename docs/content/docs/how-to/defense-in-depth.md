...
- **No per-service `caddy_extra_json:` needed.** `proxy.global_handlers:` runs both plugins for every routed service automatically. Add a new app to `services:` and it's protected — no copy-paste of plugin config. (For per-service customisation — a different rate-limit zone per app, say — `caddy_extra_json:` still works alongside.)
