...
description: Provision a Hetzner cx23, obtain a real Let's Encrypt cert via nip.io, and reach a live HTTPS endpoint in approximately 90 seconds.
weight: 5
---
...
### Register the freshly-provisioned host with yoink [step]
...
# issuer=C=US, O=Let's Encrypt, CN=E8 [step]
```
</Callout>
...
- [Reverse proxy guide](/docs/guide/proxy) — the bundled Caddy proxy and how to add HTTPS to any service.
- [Sealed secrets workflow](/docs/how-to/sealed-secrets-workflow) — managing the AGE identity and sealed SSH key this quickstart generates.
- [Standalone (no-registry) deploys](/docs/how-to/standalone-mode) — how `--build` ships the image to the host without a registry.
- [First deploy](/docs/start/first-deploy) — the minimal version of this flow without the Hetzner provisioning steps.
