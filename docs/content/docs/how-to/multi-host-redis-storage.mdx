...
description: Share ACME certificate state across hosts via Redis to avoid Let's Encrypt rate limits on multi-host setups.
weight: 16
---
...
If you're using Cloudflare's edge, the simpler answer is [Origin Certificates](/docs/how-to/cloudflare-origin-certs) with no ACME at all. This recipe is for the LE-direct case.
...
### Bake the plugin into caddy [step]
...
### Run Redis as a yoink service [step]
...
### Wire caddy's storage backend [step]
