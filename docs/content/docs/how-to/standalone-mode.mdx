...
description: Ship images directly to hosts over SSH with no registry — layer-level dedup via the bundled unregistry transport.
weight: 15
---
...
1. yoink starts an ephemeral [`ghcr.io/psviderski/unregistry`](https://github.com/psviderski/unregistry) sidecar. Unregistry is an OCI registry that reads/writes the host's image store via the containerd socket with no separate blob storage; images land in `docker images` immediately on push.
