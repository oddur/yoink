...
description: Scaffold a TanStack Start app, add postgres via yoink add, deploy, and verify with yoink pf — end to end.
weight: 2
---

A TanStack Start app deploying alongside a managed postgres, built locally and shipped without a registry. Five files, three commands, working stack on the host in about ten minutes. Adding more accessories (redis, meilisearch, …) follows the same shape: `yoink add <name>`, paste the connection block.
...
The keys are the accessory's suggestion, neutral and conventional. If your app reads a different shape (e.g. `DATABASE_URL`, `PG_HOST`), rename them in the paste; yoink doesn't care which env names the consuming service uses, only that they're present.
...
### Wire the app to the accessory [step]
...
One command. The host pulls postgres from Docker Hub; the app (with its `build:` block) is built locally and shipped to the host via [unregistry](https://github.com/psviderski/unregistry) over SSH with no registry account required for the app. Both containers come up on the `yoink` network, the app's healthcheck gates the swap.
