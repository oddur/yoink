...
### Seal the operator-supplied S3 keys
...
### List which volumes to back up, then deploy
...
### Render the backups service against the RustFS endpoint
...
### Reuse RustFS credentials as the S3 keys
...
### Add a `domain:` and Caddy rewrite to `services/rustfs.yaml`
...
### Render postgres with WAL archiving on
...
### Set the recovery target and start postgres
