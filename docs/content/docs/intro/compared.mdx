...
description: Side-by-side comparison of yoink against Kamal, Kubernetes, Dokku, Coolify, and plain docker compose.
weight: 2
---
...
Kamal is the closest neighbor. Both ship a Rust/Ruby binary over SSH into hosts and drive Docker. The differences:
...
| No-registry deploy | ✓ (`yoink up --build` ships any service with a `build:` block via an ephemeral [unregistry](https://github.com/psviderski/unregistry) sidecar; only the missing layers cross the SSH wire, no registry account needed, tarball fallback) | ✗ (registry required) |
...
| Drift detection | ✓ | ✗ |
| **Driven by AI agents / CI scripts** | ✓ (everything is CLI + YAML, no clicking required) | ◐ (REST API exists, but UI is the primary surface; agents have to learn the API instead of just editing files) |
...
| Cross-host networking | None; yoink relies on docker's per-host networks and cross-host is your routing problem (Tailscale, public DNS, …) | First-class; Uncloud builds a WireGuard mesh between machines, services dial each other by name across hosts |
...
**Pick yoink when**: you already have a routing answer (Tailscale, a single edge box, public-DNS-everywhere), you don't want any always-on yoink-specific service running on the host, and the per-host docker daemon over ssh is enough surface area to manage from. The two tools converge on "small fleet of containers, opinionated defaults" and diverge on whether the deploy tool also owns the network fabric.
...
Yoink is for people who don't want a control plane.
