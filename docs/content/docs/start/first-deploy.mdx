---
title: First deploy
description: Go from a fresh VPS to a running container in five minutes using yoink init and yoink up.
weight: 2
---
...
- [Install](/docs/start/install) — install yoink before running your first deploy.
- [Architecture](/docs/guide/architecture) — how the healthcheck-gated rolling swap you just ran works internally.
- [Deploy modes](/docs/guide/deploy-modes) — the local-build path used here, plus the CI-registry and mixed alternatives.
- [Hetzner quickstart](/docs/how-to/hetzner-quickstart) — a complete walkthrough including host provisioning, HTTPS, and sealed SSH keys.
